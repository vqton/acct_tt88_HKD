...
class PhieuXuatKhoFormDialog extends ConsumerStatefulWidget {
  final PhieuXuatKho? initialPhieuXuatKho;
  final Function(PhieuXuatKho) onSave;

  const PhieuXuatKhoFormDialog({
    Key? key,
    this.initialPhieuXuatKho,
    required this.onSave,
  }) : super(key: key);

  @override
  ConsumerState<PhieuXuatKhoFormDialog> createState() =>
      _PhieuXuatKhoFormDialogState();
}

class _PhieuXuatKhoFormDialogState
    extends ConsumerState<PhieuXuatKhoFormDialog> {
  final _formKey = GlobalKey<FormState>();
  late String _soPhieu;
  late DateTime _ngayLap;
  late String _kyKeToanId;
  String? _hoTenNguoiNhan;
  String? _boPhan;
  String? _lyDoXuat;
  String? _diaDiemXuat;
  late String _trangThai;
  final List<XuatKhoChiTietItem> _chiTietList = [];

  @override
  void initState() {
    super.initState();
    if (widget.initialPhieuXuatKho != null) {
      final phieu = widget.initialPhieuXuatKho!;
      _soPhieu = phieu.soPhieu;
      _ngayLap = phieu.ngayLap;
      _kyKeToanId = phieu.kyKeToanId;
      _hoTenNguoiNhan = phieu.hoTenNguoiNhan;
      _boPhan = phieu.boPhan;
      _lyDoXuat = phieu.lyDoXuat;
      _diaDiemXuat = phieu.diaDiemXuat;
      _trangThai = phieu.trangThai;
      for (final ct in phieu.chiTietList) {
        _chiTietList.add(XuatKhoChiTietItem(
          hangHoaId: ct.hangHoaId,
          soLuongYeuCau: ct.soLuongYeuCau,
          soLuongThucXuat: ct.soLuongThucXuat,
          donGia: ct.donGia,
        ));
      }
    } else {
      ...
      child: Container(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text(
                widget.initialPhieuXuatKho == null
                    ? 'Thêm phiếu xuất kho'
                    : 'Sửa phiếu xuất kho',
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              initialValue: _hoTenNguoiNhan,
                              onSaved: (value) => _hoTenNguoiNhan = value,
                            ),
                          ),
                          const SizedBox(width: 16),
                          Expanded(
                      ...
                      TextFormField(
                        decoration: const InputDecoration(
                          labelText: 'Lý do xuất',
                          border: OutlineInputBorder(),
                        ),
                        initialValue: _lyDoXuat,
                        onSaved: (value) => _lyDoXuat = value,
                      ),
                      const SizedBox(height: 16),
                      Row(
                        children: [
                          Expanded(
                            child: TextFormField(
                              decoration: const InputDecoration(
                                labelText: 'Địa điểm xuất kho',
                                border: OutlineInputBorder(),
                              ),
                              initialValue: _diaDiemXuat,
                              onSaved: (value) => _diaDiemXuat = value,
                            ),
                          ),
                          const SizedBox(width: 16),
                          Expanded(
  ...
  Widget _buildChiTietItem(
      BuildContext context, XuatKhoChiTietItem item, int index) {
    return Card(
          ...
          children: [
            ...
            Expanded(
              child: TextField(
                ...
                controller:
                    TextEditingController(text: item.soLuongYeuCau.toString()),
                onChanged: (value) {
                  item.soLuongYeuCau = double.tryParse(value) ?? 0;
                  setState(() {});
                },
              ),
            ),
            ...
            Expanded(
              child: TextField(
                decoration: const InputDecoration(
                  labelText: 'SL thực xuất',
                  border: OutlineInputBorder(),
                  isDense: true,
                ),
                keyboardType: TextInputType.number,
                controller: TextEditingController(
                    text: item.soLuongThucXuat.toString()),
                onChanged: (value) {
                  item.soLuongThucXuat = double.tryParse(value) ?? 0;
                  setState(() {});
                },
              ),
            ),
  ...
  void _saveForm() {
    if (_formKey.currentState!.validate()) {
      ...
      final chiTietEntities = _chiTietList
          .asMap()
          .entries
          .map((e) => PhieuXuatKhoChiTiet(
                id: '${widget.initialPhieuXuatKho?.id ?? ''}_${e.key}',
                phieuXuatKhoId: widget.initialPhieuXuatKho?.id ?? '',
                hangHoaId: e.value.hangHoaId,
                soLuongYeuCau: e.value.soLuongYeuCau,
                soLuongThucXuat: e.value.soLuongThucXuat,
      ...
      final phieuXuatKho = PhieuXuatKho(
        id: widget.initialPhieuXuatKho?.id ?? '',
        soPhieu: _soPhieu,
        ngayLap: _ngayLap,
        kyKeToanId: _kyKeToanId,
        hoTenNguoiNhan: _hoTenNguoiNhan,
        boPhan: _boPhan,
        lyDoXuat: _lyDoXuat,
        diaDiemXuat: _diaDiemXuat,
        trangThai: _trangThai,
        chiTietList: chiTietEntities,
      );

      widget.onSave(phieuXuatKho);
      Navigator.of(context).pop();
    }
  }
}

class XuatKhoChiTietItem {
  String hangHoaId;
  ...
  XuatKhoChiTietItem({
    ...
    this.soLuongYeuCau = 0,
    this.soLuongThucXuat = 0,
    this.donGia = 0,
  });

  int get thanhTien => (soLuongThucXuat * donGia).round();
}
          ...
          data: (list) => ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                trailing: Text(
                    '${NumberFormat('#,###').format(hangHoa.giaBan ?? 0)} đ'),
                onTap: () {
                  onSelect(hangHoa);
                  Navigator.of(context).pop();
                },
